...
/** Execute select statement
 ...
 Executing queries returns an `<FMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should useer.
...
- (FMResultSet *)executeQuery:(NSString*)sql withVAList: (ee FMResultSet
 @see [`FMResultSet next`](<[FMResultSet next]>)
 */
...
/** Convert the supplied NSDate to NSString, using the current database formatter.
 ...
 @param date `NSDate` of date to see setDateFormat:
 @see dateFromString:
 @see stringFromDate:
 @see storeableDateFormat:
 */
